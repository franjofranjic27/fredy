...
import { ingestConfluence } from "../../pipeline/ingest.js";
...
describe("ingestConfluence", () => {
  ...
  it("initialises the collection before processing", async () => {
    ...
    await ingestConfluence(confluence as any, embedding as any, store as any, {
      spaces: ["IT"],
  ...
  it("returns zero counts when no pages exist", async () => {
    ...
    const result = await ingestConfluence(confluence as any, embedding as any, store as any, {
      spaces: ["IT"],
      chunkingOptions: defaultChunking,
    });
  ...
  it("skips pages that fail the label filter", async () => {
    ...
    expect(result.pagesProcessed).toBe(0);
    expect(store.deletePageChunks).not.toHaveBeenCalled();
  });
  ...
  it("deletes existing chunks for each processed page", async () => {
    ...
    expect(store.deletePageChunks).toHaveBeenCalledWith("page-42");
  });
